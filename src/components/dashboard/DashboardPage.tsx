...
export function DashboardPage() {
  ...
  const { ready, presets, canEditPresets, startNewDraft } = usePresets();
  ...
  async function handleRunAudit() {
    if (!file) {
      setAuditError("Upload a dissertation before running an audit.");
      return;
    }

    if (presets.length === 0) {
      setAuditError("Add at least one agent before running an audit.");
      return;
    }
    ...
    try {
      const results = await Promise.allSettled(
        presets.map(async (preset): Promise<AgentAuditResult> => {
          const formData = new FormData();
          formData.append("file", file);
          formData.append("instructions", preset.content);
          ...
          const data = await parseApiResponse<
            AnalyzeApiResponse & { error?: string }
          >(response);

          if (!response.ok) {
            throw new Error(data.error ?? `Analysis failed for ${preset.name}.`);
          }
      ...
      if (succeeded.length === 0) {
        throw new Error("All agents failed. Check your configuration and try again.");
      }

      setReport(createAuditReport(file.name, agentResults));
    } catch (error) {
      setAuditError(
            ...
            <div className="mt-5 flex items-center gap-3 rounded-xl bg-zinc-50 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-zinc-900">
                  {auditing ? "Running audit…" : "Ready to run audit"}
                </p>
                <p className="text-xs text-zinc-500">
                  {auditing
                    ? `${presets.length} agents running in parallel`
